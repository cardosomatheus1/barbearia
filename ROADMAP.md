...
**Prontidão do produto:** use a matriz abaixo. Os 143 blocos continuam registrados
...
|---|---|---|---|
| Duas telas para o mesmo trabalho, em Crescimento e em Clientes | as quatro telas existem e funcionam: `Avisos ao cliente` (lembrete e confirmação do horário), `Automações` (mensagem com motivo próprio), `Recados` (texto sem nota) e `Avaliações` (texto com nota). Cada uma é internamente coerente, e as legendas do bloco 142 já separam o que dá para separar por escrito | **fundir** cada par numa tela só. Seis avaliadores cegos em dois painéis independentes travaram nas mesmas duas perguntas — "mandar lembrete sozinho" e "ler a reclamação do cliente" — com confiança 2 de 5, e a razão é a mesma nas duas: a fronteira é *como o sistema dispara* e *se veio estrela junto*, distinções que quem opera não faz. Renomear não resolve duas telas que fazem o mesmo trabalho | sem bloco numerado: é redesenho de tela (1.082 e 763 linhas, 9 ações de formulário), não rótulo, e precisa da medição de navegador rodando — que não roda nesta máquina. Entra no bloco seguinte que puder fechar com a medição verde |
