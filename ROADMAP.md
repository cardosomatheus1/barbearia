...
|---|---|---|---|
| A barbearia nunca entra na busca do marketplace | `marketplace_listings` está de pé desde o bloco 67, com varredura de hora em hora desde o 110, e a tela de Configurações liga e desliga a vitrine | **a coordenada**. `atualizarVitrine` delista toda unidade com `latitude`/`longitude` nulas, `saveBusiness` é o único caminho que as escreve, e nenhum formulário do produto tem esse campo — `criarUnidade` também não. Então a tabela é sempre vazia: `/buscar` responde "nenhuma barbearia publicada ainda" enquanto Configurações afirma "sua barbearia aparece na busca" (§6, pergunta 6). Junto vão o botão "Como chegar", o mapa da confirmação, o `geo` do JSON-LD, o destaque patrocinado e a atribuição de receita. Digitar latitude à mão é campo que ninguém preenche | 114: entra como `GeocodingProvider` com implementação local, pelo precedente de `PaymentProvider` e `WhatsAppProvider` — o endereço que a barbearia já digita vira coordenada, e o cartão da vitrine ganha o terceiro estado ("falta o endereço no mapa"). Achado da avaliação cega do bloco 110 |
...
| 114 | A coordenada da barbearia: `GeocodingProvider` com implementação local, para a casa entrar na busca do marketplace | ⬜ |
