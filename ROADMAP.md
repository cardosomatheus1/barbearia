...
**Prontidão do produto:** use a matriz abaixo. Os 146 blocos continuam registrados
...
| WhatsApp (Baileys) | ✅ | ✅ | ⚠️ | ❌ | ❌ | `packages/crm/src/baileys/runtime.ts::RuntimeBaileys` · `apps/web/src/app/admin/whatsapp/conexao.tsx::Baileys` · `output/PLANO_BAILEYS_PRE_GO_LIVE.md::Baileys` |
| Stripe (cobrança da plataforma) | ✅ | ✅ | ✅ | ⚠️ | ⚠️ | `packages/platform/src/stripe-pagamento.ts::StripePspProvider` · `packages/platform/src/stripe.ts::https://api.stripe.com/v1` · `packages/platform/src/cobranca-reserva.ts::cobrarComReserva` |
...
| Fiscal (NFS-e) | ✅ | ✅ | ⚠️ | ❌ | ❌ | `packages/finance/src/fiscal-emissor.ts::modoFiscal` · `apps/web/src/app/admin/fiscal/page.tsx::emissorDisponivel` · `packages/finance/src/fiscal-emissor.ts::nacional` |
...
A conciliação de WhatsApp percorre todas as unidades de cada tenant, inclusive
quando uma delas falha. A prova de isolamento e continuidade está em
`packages/crm/src/whatsapp.integration.test.ts`; a homologação de cada número
na Meta continua sendo uma etapa externa.
...
|---|---|---|---|
| Retenção de ISS e salão-parceiro no emissor próprio | emissão nacional e municipal nos perfis documentados, sem retenção nem dedução de parceiro | responsável pela retenção, documentos da parceria e regras municipais | sem bloco numerado: próxima entrega fiscal, adiada expressamente pelo proprietário em 10/09/2026; depende da definição contábil das operações e homologação dos perfis |
| Hidratação instável na tela de Campanhas | o percurso *"dono monta uma campanha sem texto aprovado"* existe e cobre o caminho inteiro, do clique ao banco | a tela devolve **React #418** (*hydration failed*) de forma **intermitente** ao voltar de `?feito=criada`: duas execuções seguidas do mesmo commit, uma vermelha e uma verde, e a árvore limpa passou na mesma máquina. A campanha **é criada** — o defeito é de renderização, não de dado. O suspeito medido é o `const agora = new Date()` de `campanhas/page.tsx`, que é o único relógio da tela e alimenta o estado dos disparos. Não foi consertado aqui porque não é deste bloco e porque um conserto sem reprodução determinística é palpite: o primeiro passo é fazer o percurso falhar sempre. **Bloco 146:** o mesmo #418 passou a aparecer no percurso *"o menu não oferece o que a conta não abre"*, que abre `/admin/campanhas` uma vez por papel — então ele não está mais contido num percurso só e derruba o portão por um caminho que não fala de campanha nenhuma. Medida a taxa nesta máquina: **1 vermelha em 5 execuções** da mesma configuração, com os quatro recursos ligados. Duas hipóteses foram testadas e **descartadas**: `/admin/dia` (40 cargas limpas) e o segundo atalho do topo. O relógio de `campanhas/page.tsx:535` segue como único suspeito medido | sem bloco definido: entra no primeiro bloco que tocar Campanhas, e o critério de pronto é o percurso vermelho **antes** do conserto |
...
| 139 | O menu e a tela passam a concordar sobre o nome de cada coisa — camada 1 do que a **avaliação cega** achou. Três leitores independentes, sem acesso à documentação, receberam as mesmas 15 tarefas de "onde você clicaria?": em **7 das 15 os três responderam com confiança ≤3**, e em 3 delas discordaram sobre qual é a tela certa. Junto, uma medição que eles não podiam ver: **11 das 41 telas do menu abriam com outro nome** — "Cobrar" levava a uma página chamada "Comanda", "Auditoria" abria "Trilha", e `Preferências` não declarava `title`, então a aba do navegador dizia **"Painel"**. É a §6 pergunta 2, a regra escrita depois de "Iniciar/Começar/Sentou". A guarda que já vigiava o `<h1>` ganha o irmão que vigia o `title`, no mesmo arquivo e com a mesma regra de casamento — corte medido: 7 de 41. Mais os rótulos que os três acusaram juntos: "Recepção" (que é a moça do balcão, não o robô do site), as duas assinaturas em direções opostas de dinheiro, e os grupos `[Conta]` e `[Conta e dados]` na mesma área | ✅ |
| 140 | Camada 2 da avaliação cega: o menu deixa de ter uma área com 12 telas e três com uma. **"Atendimento" e "Gestão" deixam de existir** — a primeira porque metade dela era caixa de entrada de reclamação e o que restava pertence a quem já tem área (a fila à Agenda, a comanda ao Financeiro, os três destinos de voz do cliente aos Clientes); a segunda porque doze telas sem parentesco não são uma área, são o que sobrou. Entram **Minha barbearia** (o que a casa vende, e quem atende com o quê) e **Marca e lojas**. O Painel sai da gaveta e vira a segunda aba de Hoje, e a Nota fiscal vai para o Fechamento, ao lado do Resultado. O equilíbrio passa de 1·1·1·5·5·7·12·9 para 3·2·4·7·7·6·3·9. Os nomes **não** são os que a avaliação propôs: ela pediu "Visão geral" e "Cadastros", que são exatamente o vocabulário que o V0 removeu por ser *"como um arquiteto organiza funcionalidade, não como a recepção pensa"* — a guarda que proíbe os dois estava lá e impediu de refazer a decisão sem saber que ela já tinha sido tomada | ✅ |
| 141 | A busca global mandava o balcão para a tela **errada**, com cara de certa. `destinosDaBusca` era um `flatMap` sobre `modulo.telas` escrito dentro do casco, então as quatro telas **internas** nunca entraram — inclusive a Ficha do cliente, que guarda fotos antes/depois, preferências de máquina e barba e observações desde o bloco 74, e é a única tela sem porta no menu. Medido sobre doze palavras do balcão: **oito não devolviam nada** e **quatro devolviam a tela errada** — "foto" levava a *Fotos e marca* (o logo da página pública), "ficha" ao *Estoque*, "histórico" à *Auditoria* e "preferência" às *Preferências* de horário de funcionamento. O dono procurou foto do corte, abriu o upload de logo e concluiu que o produto não tinha. Passa a derivar de `orientacoesVisiveis`, que já percorre as duas listas, e ganha o campo `busca` para os sinônimos que não cabem na legenda | ✅ |
| 142 | O que a **segunda** avaliação cega achou — e o número que ela deu. Rodei as mesmas 15 tarefas com três avaliadores novos no menu do bloco 140: **7 de 15 travadas antes, 7 de 15 depois**, confiança média 3,31 → 3,36. A reorganização não mexeu no ponteiro, e o motivo é que renome resolve nome, e o que trava são **telas diferentes fazendo o mesmo trabalho**. Este bloco ataca os cinco travamentos que são de lugar e de legenda: o **Painel volta para o Financeiro**, colado no Resultado — faturamento e lucro são a primeira e a última linha do mesmo demonstrativo e liam-se em áreas diferentes, e pô-lo dentro de uma área chamada "Hoje" foi regressão minha, que os três chamaram de contradição no próprio botão; "Minha barbearia" vira **"A casa"**, porque prometia os dados da casa e entregava o cardápio; e as legendas de Agenda, Profissionais, Campanhas, Retenção e Painel param de disputar o mesmo assunto. Os dois travamentos que sobram exigem fundir tela, e viraram lacuna escrita | ✅ |
| 143 | Os três nomes que a terceira avaliação cega ainda acusou, e a largura que parou de ser escolhida à mão. **"A casa" vira "Cardápio e equipe"**: três de três abriram a área esperando endereço e horário de funcionamento e acharam o catálogo — o nome anterior prometia a mesma coisa errada que "Minha barbearia", e cadastrar um serviço **piorou** de 4,4,3 para 3,3,3 por causa dele. **"Painel" vira "Movimento do mês"** — os seis avaliadores das três rodadas disseram que o nome não diz o que a tela mostra —, e **"Resultado" vira "Lucro"**, porque quem não é do ramo contábil lê "resultado" como faturamento e decide sobre o número errado. **Partir o Painel foi descartado depois de ler a tela**: os sinais e as ações que dão valor a ela cruzam dinheiro e agenda, e separar destruiria justamente a seção que a justifica. E a coluna do trilho passa a `minmax(12rem, max-content)`: ela foi ajustada à mão em três blocos seguidos, sempre porque um nome de área novo não cabia por poucos pixels | ✅ |
| 144 | A caixa de entrada do cliente deixa de ser duas. *"O cliente reclamou e escreveu, onde eu leio?"* foi a única tarefa com **cara ou coroa puro** nas quatro rodadas de avaliação cega — seis leitores, confiança 2 de 5, respostas divididas —, porque a fronteira era **se veio estrela junto**, e quem reclama não sabe que fez essa escolha. As duas fontes continuam separadas no banco, porque são diferentes de verdade: recado é anônimo e pode ser de quem **nunca foi atendido**; avaliação é nota presa a atendimento concluído. O que se junta é onde se lê e se age: `O que o cliente disse` passa a listar a nota baixa com prazo correndo **acima** dos recados, e `Notas e reputação` fica com as médias e o histórico. O corte é **agir × ler**, não "com nota / sem nota". A permissão é por metade e nunca somada — `@Exige` é conjuntivo, e pedir `feedback.view` **e** `reviews.view` tiraria a tela de quem tem só uma, que é configuração natural desde que os papéis viraram editáveis | ✅ |
| 145 | A segunda fusão que a avaliação cega pediu: `Avisos ao cliente` e `Automações` viram **Mensagens automáticas**. As duas configuravam mensagem que a casa manda sozinha, e a fronteira era *o que dispara* — o horário marcado de um lado, o comportamento do cliente do outro —, que nenhum dos dois nomes carregava; seis avaliadores travaram nisso com confiança 2 de 5, e "Automações" é a palavra que descreve as duas. Duas guardas de escopo, nenhuma delas `@Exige` somado: a permissão lista as duas e o registro casa por `some()`, e o **recurso** `avisos` — interruptor da plataforma, com semântica de "e" — sai da entrada do menu e passa a ser conferido dentro da tela, senão desligá-lo esconderia as automações junto. A rota antiga vira redirecionamento: o destino saiu do menu, não do favorito de quem já usava. A fusão quebrou a medição — a lista de telas dela é escrita à mão e continuou pedindo `/admin/avisos`, que agora redireciona —, e quem cobrou foi o navegador no segundo job do CI, catorze minutos depois: `verificar-telas-medidas.mjs` passa a fazer as duas perguntas estruturais em três segundos, sem navegador | ✅ |
| 146 | A fila de envio à mão entra no registro de navegação — ela existia desde o bloco 96 e só se chegava a ela por um cartão dentro de **WhatsApp**, que é tela de configuração: lista de trabalho diário atrás de tela de setup é a §6 pergunta 1, e `destinosDaBusca` deriva do registro, então digitar "mandar à mão" não achava nada. Ela tinha **três** nomes em três lugares e passa a ter um. E entra `/admin/tudo`, o mapa do painel: o trilho mostra sete ícones e a faixa só as telas do módulo atual, então "onde fica X?" exigia clicar os sete. Agrupado por **ritmo de uso** e não por módulo — por módulo seria o mesmo menu numa página — e o ritmo sai de `molde`, que existia desde o V7 decidindo só aparência: **9** telas dão conta do dia, 14 são para decidir, 23 se ajusta uma vez. Nada foi movido, nada renomeado fora da fila: quem já aprendeu um caminho continua com ele | ✅ |
