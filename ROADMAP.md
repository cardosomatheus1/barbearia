...
| Sinal para cliente novo em horário de pico | o sinal seletivo decide por três termos da SPEC §2.12: score abaixo do limiar, ticket acima do teto e serviço que sempre exige — com a dispensa de quem tem score alto e o reembolso por antecedência | o quarto termo, `cliente_novo AND horario_de_pico`. **Não existe horário de pico no produto**: nem coluna, nem cálculo de demanda, nem tela onde a barbearia diga quais são as suas horas cheias. Escrevê-lo agora criaria um parâmetro que ninguém preenche — o defeito de `blocks` outra vez | 57 (campanhas), que é onde o heatmap de ocupação da SPEC §5.9 entra: é ele que produz a noção de hora cheia a partir de movimento medido, em vez de pedir ao dono que adivinhe. Antes disso, o termo seria uma caixa de seleção chutada |
...
| O custo do programa de fidelidade no resultado | o programa inteiro: acúmulo nos três modelos, resgate como forma de pagamento, validade, extrato append-only e o bloqueio anti-laço | a **despesa**. Quando o cliente paga R$ 5 com crédito, a receita da venda continua sendo o preço do serviço e o dinheiro que entrou é R$ 5 menor — a diferença é custo de marketing, e hoje ela não é registrada em lugar nenhum. O caixa bate (o resgate não entra na gaveta), mas o resultado do mês superestima a margem pelo valor resgatado | 52 (DRE gerencial), que é onde despesa passa a existir como conceito. Antes disso não há para onde lançar: o produto tem receita, comissão e taxa de adquirente, e nenhuma linha de custo |
...
| De quem é o número do WhatsApp: da plataforma ou da barbearia | o motor de aviso inteiro desde o bloco 20 — seis tipos, janela de silêncio no fuso da unidade, interruptor por unidade, registro de tentativa com motivo — atrás da abstração `MessagingProvider` | a decisão de **qual número envia**, que muda o onboarding e não só o código. Três caminhos: (a) número da plataforma, verificado uma vez, com a mensagem chegando em nome do produto e não da barbearia; (b) número de cada barbearia, que é muito melhor de marca e põe verificação de empresa na Meta dentro do cadastro — etapa burocrática que a barbearia pode abandonar no meio; (c) um BSP, que encurta a burocracia e cobra por mensagem | 55 (WhatsApp oficial). Não é bloqueio de código: a abstração já existe e trocar de provedor é uma linha em `apps/worker`. É decisão de produto, e adiá-la não custa nada — o que custaria é descobri-la depois de o onboarding estar pronto |
| Tokenizar o cartão do assinante | a régua de cobrança inteira: fatura por ciclo com valor congelado, escada D+1/D+3/D+7, suspensão gradual avisada aos quinze dias, cancelamento self-service e a coluna `payment_token` com a rota que a preenche — mais o contrato `CobrancaDoClubeProvider`, que separa recusa definitiva de indisponibilidade | a **origem do token**: a tela em que o assinante digita o cartão e o adquirente devolve a referência. Sem ela nenhuma assinatura tem cartão salvo, e a régua pula a cobrança sem gastar degrau — o que quita a mensalidade é o balcão registrando o Pix que viu no extrato, que é como a esmagadora maioria das barbearias do país cobra hoje | 51 (financeiro: contas a pagar/receber), junto da lacuna do contrato com a Stripe. Tokenizar exige campo de cartão no navegador do **cliente final**, e portanto o primeiro componente de cliente do produto, com PCI a reboque — e exige uma conta de adquirente por barbearia, que é a mesma dependência comercial do split. O mecanismo não foi adiado: ele existe inteiro e é exercido pelo provedor de mentira, que recusa por padrão exatamente para que a escada, a inadimplência e a suspensão sejam percorridas pelo caminho real |
...
| Levantar o limite de fiado de um cliente | o fiado existe no domínio desde o bloco 18 — razão append-only, saldo, tela `/admin/fiado` e a forma de pagamento na comanda | **a origem do dado**: `customers.credit_limit_cents` nasce em 0 e nenhuma tela ou rota o escreve, então toda venda fiada é recusada por estourar um limite que ninguém consegue levantar. É o defeito de `blocks` outra vez, e custa uma funcionalidade que a SPEC §3.10 marca como **obrigatória para migrar** do incumbente. Achado ao semear a demonstração, que foi o primeiro cliente a tentar vender fiado | 51 (financeiro: contas a pagar/receber). O campo é um número na ficha, guardado por `customers.edit`, e a tela já existe — o que o segura é a decisão de **quem** levanta limite de crédito: na barbearia de bairro quem fia é o dono, não a recepção, e isso pede a mesma conversa de `finance.discount` com `tenants.max_discount_bps` |
...
| Estornar uma venda já fechada | as **duas consequências** dela, prontas e testadas: `estornarComissaoDaComanda` (bloco 19) lança a comissão negativa no período aberto, e `estornarSplitDaVenda` (bloco 50) desfaz o repasse — cancelando a parte que não saiu e virando dívida do profissional a que já saiu, que é a "política explícita de recuperação" que a SPEC §3.5 exige por escrito | a **operação**: a tela e a rota que dizem "esta venda foi desfeita". Não existe estorno de comanda no produto — o caminho de hoje é o balcão lançar a devolução como movimento de caixa, e a comissão continuar como foi | 51 (financeiro: contas a pagar/receber), que é onde entram as contrapartidas de dinheiro que sai. Escrevê-la agora exigiria decidir sozinho o que acontece com o fiado, o pacote consumido, a fidelidade creditada e a baixa de estoque daquela venda — cinco decisões contábeis que a SPEC não toma, e que o bloco do financeiro toma de uma vez |
