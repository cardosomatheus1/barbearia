...
**Prontidão do produto:** use a matriz abaixo. Os 144 blocos continuam registrados
...
|---|---|---|---|
| Duas telas para o mesmo trabalho, em Crescimento | `Avisos ao cliente` (lembrete e confirmação do horário) e `Automações` (mensagem com motivo próprio) existem e funcionam, cada uma internamente coerente | **fundir** as duas numa tela só. Seis avaliadores cegos em quatro rodadas travaram na mesma pergunta — "fazer o sistema mandar sozinho um lembrete" — com confiança 2 de 5, porque a fronteira é *como o sistema dispara*, distinção que quem opera não faz. O par de Clientes (`Recados` × `Avaliações`) foi resolvido no bloco 144, e por um corte diferente do que esta lacuna previa: não "com nota / sem nota", e sim **agir × ler** | sem bloco numerado: é redesenho de tela (1.082 linhas, 3 ações) e precisa da medição de navegador rodando. Entra no bloco seguinte que puder fechar com ela verde |
