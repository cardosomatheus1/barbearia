...
| Conectar o WhatsApp sem copiar identificador | o canal inteiro **falando com a Meta de verdade** desde o bloco 82: `MetaWhatsAppProvider` contra a Graph API `v21.0`, ligado por `WHATSAPP_MODO=meta`, com o token de cada barbearia decifrado da chave própria, os cinco estados de template conciliados, e o canal de reserva quando ele não está disponível | o **Embedded Signup**: o botão "Conectar WhatsApp" que abre a janela da Meta e devolve `waba_id` e `phone_number_id` sozinho. Hoje a barbearia copia os dois identificadores do painel da Meta e cola no formulário | sem bloco definido, e a dependência **não é mais a verificação de empresa** — a Meta deixou de exigi-la para começar a mandar (uma conta nova sai com teto de 250 destinatários únicos por dia, que é folgado para uma barbearia). O que falta são duas coisas: **credenciais de plataforma** (`META_APP_ID`, `META_APP_SECRET`, `META_CONFIG_ID`) de um app que precisa ser criado uma vez, e o **primeiro JavaScript que este produto manda ao navegador** — o `FB.login` roda no cliente. Não é becos sem saída: o Embedded Signup escreve nas **mesmas duas colunas** que o formulário, então ele vira o caminho normal e o formulário fica como escape para quem já tem os ids |
