...
| Os outros oito gatilhos de automação | o motor inteiro: onze gatilhos no vocabulário, condição, atraso, canal, conteúdo e **objetivo com janela**, as quatro proteções da SPEC §4.11 (teto, opt-out, silêncio, um por dia) e a atribuição que diz se cada automação vale o que custa. Três gatilhos varrem hoje: quem sumiu, aniversário e primeiro atendimento | a **consulta de varredura** dos outros oito. Cada um é uma consulta com a mesma forma — quem é, quando o fato aconteceu, qual a chave — e alguns dependem de dado que já existe (`cancelamento`, `avaliacao_positiva`), outros de leitura que ainda não foi escrita (`pacote_acabando`, `assinatura_vencendo`) | 57 (campanhas), que é onde os filtros de público entram: `churn score`, faixa de gasto e frequência são a mesma classe de consulta, e escrevê-las juntas evita oito formas diferentes de perguntar a mesma coisa. Enquanto isso os gatilhos aparecem na tela **marcados como não disparando** — escondê-los faria a SPEC §4.11 parecer entregue inteira |
