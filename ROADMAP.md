...
| Estorno do crédito em dinheiro | descer de plano gera crédito em centavos na assinatura, que abate a próxima mensalidade automaticamente | devolver o valor ao cartão de quem desceu e não quer esperar o mês virar | 29 (billing: integração com PSP, conciliação): estorno é operação do adquirente, e não existe adquirente no produto até lá. Emitir fatura de valor negativo seria a alternativa errada — documento que ninguém sabe pagar, e que o extrato do cartão não tem linha para receber |
| Débito automático da fatura | a régua emite, avisa, marca vencida, retenta na escada e suspende; o `CobrancaProvider` é a abstração que a tentativa atravessa | cobrar de verdade: hoje o provedor em produção é o `CobrancaManualProvider`, que nunca aprova, e quem quita é o Super Admin registrando o pagamento que viu no extrato | 29 (billing: integração com PSP, conciliação): é o bloco que implementa a interface contra o adquirente e concilia por webhook. A régua inteira já opera sobre isso, que é como operação pequena funciona antes de integrar |
