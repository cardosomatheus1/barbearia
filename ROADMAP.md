...
|---|---|---|---|
| A gorjeta não tem dono | `orders.tip_cents` é gravada, sai do faturamento, do DRE e da base de comissão, e a taxa do adquirente sobre ela é rateada corretamente | **de quem ela é**. A SPEC §3.6 diz em três linhas: *"vinculada ao profissional que executou"*, *"aparece separada no DRE e no extrato do barbeiro"*. Não há coluna, não há linha no DRE e não há linha em `/admin/meus-numeros` — o único lugar em que a gorjeta aparece positiva é o total do dia no caixa, sem nome. Na base de demonstração são R$ 2.628,33 em 447 comandas: dinheiro de terceiro entrando na conta da casa sem rastro | sem bloco definido: exige coluna nova (`order_payments.tip_professional_id` ou rateio pelos itens), linha no DRE, linha no extrato do barbeiro e decisão sobre comanda com dois profissionais. Merece bloco próprio. Achado da varredura geral do bloco 123 |
| O assistente responde 30 dias e o link manda para uma tela que mostra o mês | o assistente imprime o período que usou, e o catálogo de métricas leva cada resposta à tela onde o número se confere | **o período viajar no link**. A janela do assistente é 30 dias corridos (decisão escrita em `interpretacao.ts`) e o painel só tem Hoje / 7 dias / mês-calendário: "faturei R$ 33.297" leva a uma tela que diz R$ 22.947. O comentário do próprio catálogo diz que o link existe para que *"o dono clica, vê o mesmo número na tela de verdade, e passa a confiar"* | sem bloco definido: o conserto é o painel aceitar `de`/`ate`, e isso muda a assinatura de cinco consultas de `painel.ts` mais o seletor de período da tela. Merece bloco próprio com medição. Achado da varredura de §6 pergunta 6 do bloco 122 |
| A simulação dos três modelos de comissão de clube diz "nenhum atendimento" com 16 assinantes atendidos | `rentabilidadeDoClube` conta `club_uses` e lista assinante a assinante; a simulação existe e sabe comparar os três modelos | **as duas metades lerem o mesmo fato**. `simulacaoDaAssinatura` conta `commission_entries WHERE club_subscription_id IS NOT NULL`, e `lancarComissoes` só grava linha quando o item tem profissional **e** uma regra casa — então toda barbearia sem regra cadastrada vê "não há o que comparar" ao lado da tabela que prova que há. A SPEC §3.4 exige a simulação sobre os dados reais **antes** de escolher o modelo | sem bloco definido: decidir se a simulação passa a derivar de `club_uses` (o fato) ou se a tela diz que falta regra de comissão. As duas são defensáveis e a escolha muda o número. Achado da varredura de §6 pergunta 6 do bloco 122 |
| O atendimento concluído continua ocupando a agenda até o fim da reserva | a constraint de exclusão `appointments_no_overlap` protege a agenda desde o bloco 1, e `completed_at` é gravado desde a migração 0014 | **encolher a janela ocupada até `completed_at`**. Um corte de 18:15–19:35 concluído às 18:36 segura a cadeira por mais uma hora: o painel da fila diz "Ruan — livre agora" e "Sentou" na mesma tela responde *"este profissional tem cliente marcado nesse horário"* — duas telas discordando do mesmo fato (§6, pergunta 6), sobre a transição mais frequente do balcão. E o conserto que a frase sugere é impossível: `rescheduleAppointment` recusa `completed`. É a quarta vez que o padrão "não desperdiçar capacidade" aparece, e é o caso mais caro: acontece toda vez que um corte termina antes da hora | sem bloco definido: mexe na constraint de exclusão, que é a garantia mais forte deste schema, e a forma segura é `LEAST(ends_at, GREATEST(starts_at, completed_at))` — **encolher nunca, estender jamais**, senão concluir um atendimento que passou da hora seria recusado pelo horário do cliente seguinte. Precisa de bloco próprio, com teste de corrida e medição. Achado da varredura de estado sem saída do bloco 121 |
| A plataforma não cancela, não reativa e não troca o cartão de uma barbearia | as rotas existem inteiras — `POST`/`DELETE .../cancelamento`, `GET`/`PUT .../cobranca`, `POST .../estorno`, mais `GET /assinaturas` — com guarda, `@AgeNaConta` e trilha | **as telas**. Nenhuma delas tem cliente em `plataforma-api.ts`, e `subscriptions.status` não é mostrado em lugar nenhum do painel da plataforma. Do outro lado, `/admin/plano` diz ao cliente em letras *"para trocar o cartão, fale com o suporte"* — e o suporte não tem essa tela: é `curl` ou `UPDATE`. A metade irmã funciona (a fatura tem "Registrar pagamento" e "Anular"), o que torna a ausência mais fácil de não notar | sem bloco definido: é tela de plataforma, e o produto tem uma barbearia paga hoje. Entra junto do primeiro bloco que mexa no painel de assinaturas da plataforma. Achado da varredura de estado sem saída do bloco 121 |
| O menu do painel não esconde o que a guarda recusaria | `modulosVisiveis` já filtra por recurso da plataforma, e a `PermissaoGuard` recusa de verdade em toda rota | **o filtro por permissão**. O barbeiro vê os 39 destinos, e sete recusam ao abrir — entre eles Resultado, Privacidade e Usuários e acessos. O comentário de `admin.controller.ts` afirma que "o menu esconde o que a guarda recusaria", e é a mesma regra que o `CLAUDE.md` já cobra para catálogo de métricas | sem bloco definido: o conserto tem que ser **derivado**, não uma coluna `permissao` escrita à mão em 39 linhas de `secoes.ts` — essa seria a sexta lista paralela deste código, e errar uma linha esconde uma tela de quem deveria vê-la. Precisa de guarda que leia o `@Exige` da rota que cada tela consome. Achado da avaliação cega do bloco 110 |
...
| Leitura consolidada da rede | `TODAS_AS_UNIDADES`, `ehConsolidado` e `EscolhaDeUnidade` existem em `packages/core/src/multiunidade.ts` desde o bloco 58, com teste próprio, e o cabeçalho do arquivo promete: *"o relatório é o oposto: o dono quer as duas juntas"* | o **chamador**. Nenhuma tela lê a rede por esse mecanismo — as que somavam a rede o faziam por acidente, e o bloco 117 fechou as quatro (comissão, estoque, margem, CMV). O dono de uma rede hoje troca de loja para ver cada uma | sem bloco definido: é `blocks` outra vez — motor aceito e nunca preenchido —, e `varredura-com-chamador.test.mjs` não alcança porque só olha função com prefixo `varrer|atribuir|expirar`. O que segura a decisão é a tela: consolidado exige um seletor com três estados (esta loja, aquela, todas) no casco, e o casco só ganhou o **nome** da loja no 117. Entra junto do seletor, e a guarda de chamador precisa ser ampliada no mesmo bloco, senão a lacuna se repete |
...
| Revogar foto pelo autoatendimento do cliente | as duas autorizações existem, são conferidas na escrita e por gatilho, e revogar **apaga** as fotos — inclusive o que já estava no portfólio. O balcão registra a decisão pela ficha, e a anonimização alcança a foto | a rota do **titular** para revogar sozinho: hoje a de autoatendimento é fixa em `marketing`, e para foto quem revoga é a barbearia a pedido dele | sem bloco definido: a superfície de autoatendimento do cliente é uma tela só (`/{slug}/meus-agendamentos`) e nunca teve gestão de consentimento. Vale entregar as quatro finalidades de uma vez, com o texto de cada uma, e não a de foto sozinha — meia tela de consentimento é pior que nenhuma, porque ensina que ali só dá para revogar uma coisa |
