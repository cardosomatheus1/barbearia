...
|---|---|---|---|
| Passar um pacote para outra pessoa | a coluna `transferable` existe no catálogo e é **congelada em cada compra**, com o cadastro na tela de Pacotes e o padrão desligado — que é o comportamento anterior e o mais conservador, já que um pacote transferível é um vale ao portador | a operação em si: dizer *para quem* vai, o que exige achar a outra pessoa, provar que ela concorda e mover as unidades restantes sem quebrar a receita já reconhecida na compra do primeiro dono | 52 (financeiro: vale, DRE gerencial). Muda do 51 com o motivo escrito: o que o 51 entregou foi o razão do **fiado**, e transferir pacote não passa por ele — o que a operação move é **receita diferida** de uma pessoa para outra, e o número que precisa continuar certo é o do resultado, que é o do 52. Escrevê-la aqui significaria decidir sozinho o que acontece com a receita já reconhecida, sem o relatório que a mostraria errada |
...
| Estornar uma venda já fechada | as **duas consequências** dela, prontas e testadas: `estornarComissaoDaComanda` (bloco 19) lança a comissão negativa no período aberto, e `estornarSplitDaVenda` (bloco 50) desfaz o repasse — cancelando a parte que não saiu e virando dívida do profissional a que já saiu, que é a "política explícita de recuperação" que a SPEC §3.5 exige por escrito | a **operação**: a tela e a rota que dizem "esta venda foi desfeita". Não existe estorno de comanda no produto — o caminho de hoje é o balcão lançar a devolução como movimento de caixa, e a comissão continuar como foi | 52 (financeiro: vale, DRE gerencial). Muda do 51 com o motivo escrito: o 51 entregou o que a casa deve e tem a receber, e a venda desfeita não é uma conta — ela toca fiado, pacote consumido, fidelidade creditada, baixa de estoque e caixa, cinco contrapartidas contábeis de uma vez. O relatório que mostra cada uma delas errada é o DRE do 52, e operação e conferência precisam nascer juntas: uma venda estornada que continua no resultado do mês é pior que não poder estornar |
