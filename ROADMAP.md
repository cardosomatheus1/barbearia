...
| DRE por unidade numa rede | o relatório inteiro, com as oito linhas da SPEC §3.10, comparativo contra janela do mesmo tamanho e cinco das seis consultas recortadas por `location_id` — direto ou pelo `orders` de onde vieram | o recorte da **mensalidade do clube**: `club_invoices` não tem unidade, porque a assinatura é do cliente com a barbearia e não com uma loja. Atribuí-la a uma delas hoje seria inventar o dado | 58 (multiunidade: seleção, consolidação). Enquanto o produto tem uma unidade só, a linha não muda número nenhum — `primaryLocation` resolve sempre a mesma. É no bloco que introduz a segunda loja que a decisão precisa ser tomada, e ela é de negócio: a mensalidade é receita da unidade onde a pessoa mais se atende, é rateada, ou fica fora do DRE por unidade |
