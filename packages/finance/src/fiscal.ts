...
export async function pedirNota(
  ...
  const vendas = await tx.$queryRaw<
    { status: 'open' | 'paid' | 'cancelled' | 'refunded'; customer_name: string | null }[]
  >`
    SELECT o.status::text AS status, c.name AS customer_name
      FROM orders o
  ...
  const criadas = await tx.$queryRaw<{ id: string }[]>`
    INSERT INTO fiscal_invoices
      (tenant_id, location_id, order_id, regime, service_cents, partner_cents,
       iss_bps, service_code, municipality_ibge, customer_name, created_by,
       created_by_name)
    VALUES (
      NULLIF(current_setting('app.tenant_id', true), '')::uuid,
      ...
      ${venda.customer_name}, ${params.staffId}::uuid, ${params.staffName}
    )
    RETURNING id
  `;
