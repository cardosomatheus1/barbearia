...
import os, subprocess, time, urllib.request, uuid, tempfile, base64
...
env.update({'DATABASE_URL': 'postgres://barbearia_app:' + quote(env['APP_DB_PASSWORD'], safe='') + '@127.0.0.1:5432/' + db,
    ...
    'FISCAL_MUNICIPAL_BIN': str(root/'integrations/fiscal-municipal/publish/Barbearia.FiscalMunicipal'),
...
processes = []; logs = []
confianca = tempfile.TemporaryDirectory(prefix='nfse-confianca-browser-', dir=runtime)
env['FISCAL_CONFIANCA_DIR'] = confianca.name
# Chave privada somente sintética; o diretório temporário é removido no finally.
subprocess.run(['node', '--import', './apps/api/node_modules/tsx/dist/loader.mjs',
    'output/preparar_confianca_fiscal.mjs'], cwd=root, env=env, check=True)
pki = __import__('json').loads((Path(confianca.name)/'a1-teste.json').read_text())
env['FISCAL_AUTORIDADES_PEM_B64'] = base64.b64encode(pki['autoridadePem'].encode()).decode()
del pki
...
try:
    ...
    subprocess.run(['node', '--import', './apps/api/node_modules/tsx/dist/loader.mjs', 'output/prova_fiscal_navegador.mjs'], cwd=root, env=env, check=True)
finally:
