"""Prova Meta com API e web reais; transporte sintético sem rede Meta."""
...
db = 'barbearia_meta_browser_' + uuid.uuid4().hex[:10]
env.update({'DATABASE_URL': 'postgres://barbearia_app:' + quote(env['APP_DB_PASSWORD'], safe='') + '@127.0.0.1:5432/' + db,
    ...
    'META_APP_ID': '123456789', 'META_APP_SECRET': 'segredo-sintetico-sem-valor', 'META_CONFIG_ID': '987654321', 'WHATSAPP_ONBOARDING': os.environ.get('META_ENSAIO_MODO', 'padrao'), 'BAILEYS_HABILITADO': '0', 'WHATSAPP_TOKEN_KEY': __import__('base64').b64encode(os.urandom(32)).decode(), 'FISCAL_MODO': 'nenhum', 'FISCAL_SECRET_KEY': __import__('base64').b64encode(os.urandom(32)).decode(),
...
try:
    ...
    for name, args, cwd in [('api', ['node', 'apps/api/dist/main.js'], root),
        ...
        log = (runtime/('meta-browser-' + name + '.raw.log')).open('w'); logs.append(log)
    ...
    subprocess.run(['node', 'output/prova_meta_navegador.mjs'], cwd=root, env=env, check=True)
finally:
