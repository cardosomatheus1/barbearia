"""Prova Baileys com API, web e main do worker reais; socket sintético sem rede WhatsApp."""
...
env.update({'DATABASE_URL': 'postgres://barbearia_app:' + quote(env['APP_DB_PASSWORD'], safe='') + '@127.0.0.1:5432/' + db,
    ...
    'BAILEYS_HABILITADO': '1', 'BAILEYS_ENSAIO_LOCAL': '1', 'WORKER_INTERVALO_MS': '250', 'WHATSAPP_TOKEN_KEY': __import__('base64').b64encode(os.urandom(32)).decode(), 'FISCAL_MODO': 'nenhum', 'FISCAL_SECRET_KEY': __import__('base64').b64encode(os.urandom(32)).decode(),
...
try:
    ...
    for name, args, cwd in [('worker', ['node', '--import', './output/baileys_worker_registrar.mjs', 'apps/worker/dist/main.js'], root), ('api', ['node', 'apps/api/dist/main.js'], root),
        ('web', ['node', str(root/'apps/web/node_modules/next/dist/bin/next'), 'start', '-p', '3481', '-H', '127.0.0.1'], root/'apps/web')]:
        log = (runtime/('baileys-worker-browser-' + name + '.raw.log')).open('w'); logs.append(log)
    ...
    subprocess.run(['node', 'output/prova_baileys_worker_navegador.mjs'], cwd=root, env=env, check=True)
finally:
