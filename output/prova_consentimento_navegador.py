"""Prova do aceite de cadastro com API/web reais e OTP sintético sem WhatsApp."""
...
db = 'barbearia_consentimento_browser_' + uuid.uuid4().hex[:10]
env.update({'DATABASE_URL': 'postgres://barbearia_app:' + quote(env['APP_DB_PASSWORD'], safe='') + '@127.0.0.1:5432/' + db,
    ...
    'BAILEYS_HABILITADO': '0', 'WHATSAPP_TOKEN_KEY': __import__('base64').b64encode(os.urandom(32)).decode(), 'FISCAL_MODO': 'nenhum', 'FISCAL_SECRET_KEY': __import__('base64').b64encode(os.urandom(32)).decode(),
    'PSP_MODO': 'nenhum', 'WHATSAPP_MODO': 'nenhum', 'NODE_ENV': 'test',
    'RATE_LIMIT_SHORT': '100000', 'RATE_LIMIT_LONG': '100000'})
temporario = tempfile.TemporaryDirectory(prefix='barbearia-consentimento-', dir='/tmp')
env['CONSENTIMENTO_ENSAIO_OTP'] = temporario.name + '/otp.json'
processes = []; logs = []
...
try:
    ...
    for name, args, cwd in [('api', ['node', 'output/consentimento_api_piloto.mjs'], root),
        ('web', ['node', str(root/'apps/web/node_modules/next/dist/bin/next'), 'start', '-p', '3491', '-H', '127.0.0.1'], root/'apps/web')]:
        log = (runtime/('consentimento-browser-' + name + '.raw.log')).open('w'); logs.append(log)
    ...
    else: raise RuntimeError('Pilha manual local não ficou pronta')
    subprocess.run(['node', 'output/prova_consentimento_navegador.mjs'], cwd=root, env=env, check=True)
finally:
