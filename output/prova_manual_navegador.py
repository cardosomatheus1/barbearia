...
db = 'barbearia_manual_browser_' + uuid.uuid4().hex[:10]
...
try:
    ...
    for name, args, cwd in [('api', ['node', 'apps/api/dist/main.js'], root),
        ...
        log = (runtime/('manual-browser-' + name + '.raw.log')).open('w'); logs.append(log)
    ...
    subprocess.run(['node', 'output/prova_manual_navegador.mjs'], cwd=root, env=env, check=True)
finally:
