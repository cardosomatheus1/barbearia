...
| Criar e editar | é o mesmo formulário, escrito uma vez. A segunda cópia dentro da linha é o caminho curto e vira a lista paralela de sempre: o campo que o bloco seguinte acrescentar aparece em criar e não em editar, com a suíte verde, porque cada metade sozinha continua coerente. O `PUT` já é do objeto inteiro e a ação já recebe `id` |
| Formulário desenhado uma vez por linha | prefixa o `id` dos campos. `htmlFor` casa com `id`, e `id` é único no **documento**, não no formulário: com `id="nome"` literal, clicar no rótulo da terceira linha foca o campo da primeira. Salvar continua certo — o `name` é do formulário —, e por isso o defeito é invisível para quem usa mouse e só aparece para quem usa teclado. Radio é o contrário e não precisa de nada: o dono do grupo é o `<form>` |
| Guarda que lê o fonte de uma tela | vê o que está **escrito**, nunca o que é desenhado. Um componente escrito uma vez e renderizado dez vezes aparece uma vez na varredura: a guarda de `id` repetido fica verde se o prefixo for removido, e isso foi verificado quebrando de propósito. O limite vai escrito na própria guarda — guarda em que se confia mais do que ela alcança é pior que guarda nenhuma |
