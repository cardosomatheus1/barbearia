...
| Escopo de leitura que soma a rede | é `EscolhaDeUnidade`, nunca `string \| null`. Nulo é indistinguível de "o chamador esqueceu", que é como oito leituras passaram a somar a rede sem ninguém ter decidido isso (bloco 117); `'todas'` é uma palavra que só se escreve de propósito. E o consolidado é de **leitura**: caixa, comanda e agenda continuam de uma loja, e somá-las faria a recepção fechar o caixa da loja errada |
| Seletor de "todas as lojas" | mora na tela do relatório, nunca no casco. No casco ele valeria para as vinte telas do painel de uma vez, e a primeira a somar duas gavetas seria defeito de dinheiro. Quem pode pedir a rede é quem enxerga todas as lojas — e o gerente escopado a uma filial recebe a filial dele, sem erro: ele está vendo tudo o que pode ver |
| Guarda que reprova o legítimo | é guarda que alguém desliga, e por isso o corte se mede **antes** de escrever. "Todo símbolo exportado de `core` usado fora do próprio arquivo" acusa 429 de 1014; estreitado para constante em CAIXA_ALTA, ainda 78 de 182 — e a maioria é parâmetro de calibragem exportado para o próprio teste, que não é defeito. Sem corte defensável, a saída é a lacuna escrita com os números, não a guarda que se desliga na primeira semana |
| Meia tela de consentimento | é pior que nenhuma: ela ensina que ali só dá para revogar uma coisa, e o titular para de procurar. O autoatendimento mexia só em `marketing`, e quem quisesse tirar a própria foto do Instagram da barbearia dependia de pedir ao balcão — que depende de alguém lembrar |
| Uma finalidade por chamada | e nunca as três num corpo só. Cada decisão grava a **versão do texto daquela finalidade**, e um corpo com as três juntas gravaria três linhas sob um clique só, sem dizer qual texto a pessoa leu — que é exatamente o que a versão existe para provar |
| Finalidade que vem do formulário | é conferida contra o catálogo, como a versão. O que chega do navegador é palpite; `service` não tem aceite a revogar — é o tratamento necessário para executar o contrato —, e oferecê-lo faria o cliente "revogar" algo que a revogação não desliga |
| Data formatada numa tela do servidor | leva `timeZone` da unidade, e quando pede **hora** isso deixa de ser só o defeito D2: `Intl` usa UTC no servidor e o fuso do aparelho no navegador, o React não reidrata a diferença e a **página inteira** cai com o erro 418. A lista de mensagens da ficha nasceu assim, com um comentário dizendo em letras que o fuso do navegador era o certo — e o portão passou verde nas duas voltas. Quem pegou foi o percurso da medição |
| Corte de uma guarda nova | é **medido antes de escrevê-la**, e o número vai no commit. "Toda formatação sem `timeZone`" acusava 42 de 63 chamadas — guarda que reprova o legítimo é guarda que alguém desliga na primeira semana. "Pede hora e não diz o fuso" acusa 8, e as 8 eram defeito: o mesmo assunto, um corte defensável, e as 34 que mostram só data ficam de fora **de propósito** — e são pegas no dia em que uma delas passar a pedir hora |
| Fuso numa tela que atravessa barbearias | é constante nomeada em `core` (`FUSO_DA_PLATAFORMA`), nunca literal na página nem `locations.timezone` disfarçado. O painel da plataforma não tem unidade de onde tirar o fuso — e foi por isso que ele ficou sem nenhum —, mas quem lê é **uma** organização, não mil e duzentas |
| Frase que nomeia onde outra coisa está | não. "O motivo está ao lado" erra na primeira largura em que o layout reflui, e ele reflui a partir de 360px — que é o piso, não a exceção. E ali o motivo era obrigatório por `CHECK`: repetir que ele existe não acrescentava nada, e o que faltava a quem lê era o que fazer com ele |
| Transporte de uma mensagem | é do **texto**; conexão é da **unidade**. Achatados numa pergunta só — "Como você quer enviar?" com Meta, Baileys e Manual lado a lado —, o produto escondeu duas capacidades inteiras: os textos do Baileys só apareciam com aquela conexão escolhida, e os manuais moravam noutra página. Manual convive com qualquer conexão e nunca sai sozinho; tratá-lo como conexão fez dele o único cartão que levava para fora da tela |
| Separar não é esconder | os três grupos aparecem sempre, com a condição escrita ao lado — *"só saem sozinhas com a conexão Baileys"* —, nunca sumindo conforme o modo ativo. Escondido, o recurso não existe para quem nunca trocou a conexão para descobri-lo, e é o mesmo defeito do gatilho que aparece marcado em vez de oculto |
| Um campo que responde duas perguntas | ganha o segundo campo. `disponivel` respondia *"está pronto?"* e *"dá para usar hoje?"* ao mesmo tempo, e o editor do Baileys passou a chamar de "Pausada" toda mensagem de quem estava na conexão Meta — sobre um texto que a barbearia tinha deixado ligado. Conflacionar foi o defeito original e por pouco não foi o conserto |
| Atribuição de receita | conta quem **recebeu**, nunca quem foi carimbado. `sent_at` é gravado mesmo com o canal desligado — o bloco 97 achou isso e consertou só a contagem da tela —, então a atribuição creditava quem voltou sozinho: R$ 1.991,00 numa campanha de zero mensagens entregues, na coluna que a própria tela chama de "a única que responde se ela pagou o que custou" |
| Âncora de um botão | cai na metade que **age**, e há guarda. A seção da ficha ganhou um histórico acima dos botões e o `id` do âncora ficou no `h2`: o botão "WhatsApp" do cabeçalho passou a levar a quem já mandou, e não a quem manda. É o título que não descreve o que está embaixo dele, com um botão apontando para o erro |
| `accepted` do provedor não é `delivered` | e o produto não pode chamar os dois de "enviada". A Cloud API responde `accepted` com um id: a Meta **aceitou para entregar**, e quem conta o desfecho é o webhook. `notifications.status='sent'` e a frase "Mensagem enviada pelo WhatsApp da casa" afirmavam a segunda coisa sabendo só a primeira — quatro mensagens aceitas, `delivered_at` nulo nas quatro, e a recepção sem nada para olhar. Duas horas de investigação, e a resposta estava sendo escrita em `whatsapp_messages` desde o bloco 58 **sem nenhum leitor** |
| Porteiro de handler que decide não fazer nada | devolve o motivo, nunca `return` puro. Seis handlers do worker cortavam o envio quando o recurso estava desligado e fechavam a tarefa como `concluida`: nem notificação, nem log, nem rastro. É a regra que já valia para `notifications.reason` — *"nada foi enviado" sem motivo transforma toda pergunta do dono numa investigação* — e que não valia para a fila. O laço publica `fase: 'pulada'` com o código, e a guarda é derivada da chamada a `recursoLigado`, não de uma lista de handlers |
| Tarefa periódica de um caminho que a instalação não contratou | não é agendada. `FISCAL_MODO` nasce `nenhum` e o laço enfileirava `fiscal.conciliar` para toda barbearia de hora em hora, com `exigirEmissorFiscal()` lançando: três `tarefa.falhou` por hora, nas duas barbearias, desde sempre. Nada quebrava — e é justamente esse o estrago, porque vermelho constante e inofensivo é o que ensina quem opera a não olhar o log |
| Função de integração com **um** chamador | é caminho que só metade das pessoas percorre. `assinarWebhook` existia desde o bloco 88, com o comentário certo — *"sem isto a barbearia manda mensagem e não recebe nada de volta"* — e era chamada só pelo Embedded Signup. Quem cadastrou o número pelo formulário ficou com `subscribed_apps` vazio e a Meta nunca contou desfecho nenhum. Escrita, correta, e fora do caminho que as pessoas usam |
| Viagem a serviço de terceiro dentro de uma requisição | é tarefa de fila, e o precedente é a nota fiscal: emissão nunca bloqueia a venda. Medido, o `POST` que submetia texto à Meta levava **7.039 ms** contra o teto de 10 s que o `web` impõe — e estourar não é só lentidão: o `web` aborta e mostra recusa, a Meta **já recebeu**, e a tentativa seguinte bate em "nome repetido". Uma operação que diz "não deu" sobre o que deu, e cujo conserto natural produz o erro seguinte |
| Estado em voo que a fila cria | precisa da varredura para quando a tarefa desiste. A escada é 1, 2, 4, 8 e 16 minutos: em meia hora ela para, e a linha fica no estado que **recusa a submissão seguinte** — a barbearia perde o texto e o caminho de refazê-lo ao mesmo tempo, sem erro e sem alerta. A soltura só alcança quem nunca chegou ao terceiro (`meta_id IS NULL`): quem chegou é da conciliação por nome, e soltá-lo criaria a segunda submissão do que já existe lá |
| Um estado que passa a significar duas coisas | ganha o campo que os separa, obrigatório no tipo. Com a ida à Meta na fila, `pendente` virou *esperando a fila* **e** *a Meta está avaliando* — e a tela dizia "a Meta costuma responder em minutos" sobre um texto que ela nunca viu, mandando a barbearia procurar no painel dela o que não estava lá. Rótulo e explicação saem **juntos** da mesma função do domínio: separados, o estado novo faria os dois mapas divergirem e a tela mostraria "Na fila" ao lado de "a Meta já respondeu" |
| Estado vazio de uma lista já recortada | diz **qual** vazio é o dela. Três telas escreviam "Nenhum texto aprovado" para dois fatos diferentes — a Meta não aprovou nada, ou aprovou e nenhum é do tipo que aquela tela manda —, porque o recorte por tipo tinha sido somado ao filtro por estado e apagava a diferença antes de a tela existir. A barbearia com dois aprovados leu a frase, foi ao painel da Meta, viu os dois lá e concluiu que o produto estava quebrado. As três eram internamente coerentes e nada ficava vermelho: o que faltava era **a diferença entre o que ela recebeu e o que existe**, e ela só existe se as duas contagens chegarem separadas |
