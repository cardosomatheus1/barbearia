...
| Estado que o enum tem e nenhum caminho escreve | é estado que não existe, e a linha que precisaria dele fica presa. `order_status` tem `cancelled` desde a migração 0018 e **nada no produto o escrevia**: a comanda aberta por engano só saía de `open` sendo paga, e comanda vazia não fecha — o fechamento exige pelo menos uma forma de pagamento. O botão que a cria fica a um clique dentro de um `details`, e depois de criada ela não aparecia em tela nenhuma: quem fechasse a aba perdia a única porta |
